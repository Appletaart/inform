...
export default class BesturenController extends Controller {
 ...
 // @tracked toggleClick;
  ...
  /* 
  queryParams = ['municipalities'];

  @tracked municipalities = null; 

  @action
  changeMunicipalities(id){
    this.municipalities = id;
  }
  */
 ...
 @action toggleClose() {
  ...
  // with mandaten.json
  /* get getBestuur() {
    let bestuurseenheids = d3.group(this.model.data, d => d.bestuurseenheidLabel);
    ...
    if (!burgemeester) {
      ...
      burgemeester.forEach(burgemeesters => {
        ...
        if (+period_Start <= +bestuur_Start && !(+bestuur_End <= +today)) {
          eenBurgemeester.push({ voornaam: burgemeesters.voornaam, achternaam: burgemeesters.achternaam, functie: burgemeesters.bestuursfunctieLabel, fractie: burgemeesters.fractieNaam })
  ...
  get getBestuur() {
    let bestuurseenheids = d3.group(this.model.realdata, d => d.bestuurseenheidnaam);
    let een_bestuurseenheid = []
    for (const [key] of bestuurseenheids) { een_bestuurseenheid.push(key) }
    let bestuurseenheid = bestuurseenheids.get(this.gemeente);
    ...
    if (!burgemeester) {
      ...
      // console.log({ een_bestuurseenheid, een_burgemeester, een_schepenen, een_tschepenen, een_voorZgr, een_gr });
       return {een_burgemeester, een_schepenen, een_tschepenen, een_voorZgr, een_gr }  
    }// eind getBestuur

  get getBestuurSearch() {
      let bestuurseenheids = d3.group(this.model.realdata, d => d.bestuurseenheidnaam);
      let een_bestuurseenheid = []
      for (const [key] of bestuurseenheids) { een_bestuurseenheid.push(key) }
